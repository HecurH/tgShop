...
class MoyNalog:
    ...
    def __init__(self, timezone_shift = None):
        ...
        self.timezone_shift = timezone_shift or 3
        self.profile = ProfileStorage.get()
        self.auth = {"Authorization": f"Bearer {self.profile.token}"}
    ...
    def get_receipt_url(self, approvedReceiptUuid):
        return f" https://lknpd.nalog.ru/api/v1/receipt/771551648410/{approvedReceiptUuid}/print"
    ...
    def create_invoice(self, operationTime,
        ...
        totalamount = (
            sum(el.amount * el.quantity for el in svs)
            if len(svs) > 1
            else svs.amount * svs.quantity
        )
