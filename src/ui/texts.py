...
class AdminTextGen:
    ...
    def price_confirmation_text(entries: list[CartEntry], ctx: Context):
        ...
        next_input_info = "\n".join(f"{idx}: {json.dumps([f'{key}-{option.get_chosen().price.model_dump_json()}' for key, option in entry.configuration.get_price_blocking_options().items()], ensure_ascii=False)}" for idx, entry in enumerate(entries))
