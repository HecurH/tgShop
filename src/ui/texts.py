...
class AdminTextGen:
    ...
    def price_confirmation_text(entries: list[CartEntry], ctx: Context):
        ...
        next_input_info = "\n".join(f"{idx}: {json.dumps([f'{key}-{option.get_chosen().price.model_dump()}' for key, option in entry.configuration.get_price_blocking_options().items()])}" for idx, entry in enumerate(entries))
