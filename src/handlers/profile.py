...
@router.message(ProfileStates.Menu)
...
@router.message(ProfileStates.Settings.ChangeLanguage)
async def profile_change_lang_handler(_, ctx: Context) -> None:
    ...
    if ctx.message.text in SUPPORTED_LANGUAGES_TEXT.keys():
        ...
        ctx.t = TranslatorHub.get_for_lang(ctx.lang, ctx.services.placeholders)
...
@router.message(ProfileStates.Settings.ChangeCurrency)
async def profile_change_lang_handler(_, ctx: Context) -> None:
    if ctx.message.text == ctx.t.UncategorizedTranslates.back:
        await call_state_handler(ProfileStates.Settings.Menu, ctx)
        return
...
@router.message(ProfileStates.Referrals.AskForJoin)
async def referrals_ask_for_join_handler(_, ctx: Context) -> None:
    ...
    if ctx.message.text == ctx.t.UncategorizedTranslates.what_is_this:
        await call_state_handler(ProfileStates.Referrals.AskForJoin, ctx, send_before=(ctx.t.ProfileTranslates.Referrals.what_is_this, 1.5))
        return
...
@router.message(ProfileStates.Referrals.Menu)
...
async def editable_requirement_handler(_, ctx: Context) -> None:
    ...
    if len(delivery_info.service.selected_option.requirements)-1 == requirement_index:
        ...
        if await ctx.fsm.get_value("back_to_cart_after_delivery"):
            ...
            await call_state_handler(CartStates.Menu, ctx, current=await ctx.fsm.get_value("current") or 1)
            return
